...
**Branch**: `001-sera-ai-chatbot` | **Date**: 2026-09-18 | **Spec**: [spec.md](./spec.md)
...
**Primary Dependencies**: Hono 4.13.8、React 19.2.8、Vite 8.3.0、`@strands-agents/sdk` 1.18.0（先行検証で固定）、Privy React SDK / `@privy-io/node`（実装時に同一 minor へ固定）、zod、OpenAPI Generator 7.25.0、Cloudflare AI Gateway、Sera REST API

**Storage**: Cloudflare D1（ユーザー所有関係、会話、承認、冪等性、取引状態、監査イベント）、Cloudflare Workflows（確定待ち）、ブラウザメモリ（表示専用の一時状態）。KV・ローカルファイル・Worker グローバルメモリは重要状態に使わない
...
**Scale/Scope**: 教材・検証用 MVP。初期目標は同時チャット 20、登録利用者 1,000、利用者あたり会話 100・メッセージ 1,000・資産操作 100。MVP の資産操作は Sepolia の `JPYC/USDC` と両 token の transfer に限定し、対応 token/market/address/最小量は起動時または要求時に Sera `/tokens`・`/markets` から再検証する

## Constitution Check

*GATE: Phase 0 前と Phase 1 後に確認。現在は Phase 1 後の再評価結果。*
...
| III. Workers 成立性 | stdio/native 依存を排除し直接 Sera REST adapter を選択。Strands/provider/stream/signing は Phase 0 spike の必須 gate | PASS（spike 合格を実装着手条件とする） |
...
原則違反および未解決の `NEEDS CLARIFICATION` はない。Phase 0 spike が不合格の場合は後続実装を停止し、`research.md` の fallback に従って再計画する。
...
│   ├── sera-adapter.md
│   └── sse-events.md
└── tasks.md                 # $speckit-tasks で生成（本計画では未作成）
```
...
### Phase 0 — 成立性 spike（後続作業の blocking gate）
...
6. gate 記録に bundle size、startup、CPU、memory、subrequest、外部 latency、実行日時、commit/version を保存する。1つでも不合格なら Phase 1 へ進まない。

### Phase 1 — 契約・domain・基盤
...
4. proposal hash、approval version、operation state transition、idempotency transaction を pure domain として unit test する。

### Phase 2 — read-only MVP

1. Privy login と embedded wallet の create/read、所有権の同期を実装する。
...
4. AS-001〜AS-007 と user story P1/P2 を contract/integration/E2E で合格させる。

### Phase 3 — swap / transfer / status

1. deterministic service が quote/build response から proposal を作り、UI が全条件と期限を表示する。
...
5. P3〜P5、AS-008〜AS-012、期限切れ・拒否・response loss・並行クリックを testnet で検証する。

### Phase 4 — hardening と UI

1. allowlist CORS、security headers、rate limit、timeouts、bounded retry、PII/secret redaction を追加する。
2. MUI を基盤に chat、wallet、market source badge、transaction review、status timeline を responsive/accessibility 対応する。
3. Vitest、Newman、SSE parser、Playwright、Wrangler remote smoke を統合し、実測 evidence を生成する。

### Phase 5 — deploy / destroy lifecycle
...
|------|--------------|-----------------------|
| Strands が Worker runtime で非互換 | bundle/runtime で Node API error | Phase 0 で停止。browser export の範囲縮小、custom provider、最後に最小 orchestrator への置換を再計画 |
